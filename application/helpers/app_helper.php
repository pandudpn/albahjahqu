...
function print_message($str){
    $type = explode('|', $str);
    if($type[0] == 'success')
    	return 'asdf<div class="message success"><p>'.$type[1].'</p></div>';
    elseif($type[0] == 'error')
    	return '<div class="message errormsg"><p>'.$type[1].'</p></div>';
    else
    	return '';
}
...
function print_message_admin($str){
	$type = explode('|', $str);
	if($type[0] == 'success'){
		return '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>'.$type[1].'</div>';
	}elseif($type[0] == 'error'){
		return '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>'.$type[1].'</div>';
	}else{
		return '';
	}
}
